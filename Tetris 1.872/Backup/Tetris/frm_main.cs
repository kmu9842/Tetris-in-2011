...
namespace Tetris
{
    ...
    {
        internal ushort StressValue = 0, 본좌임팩트Val = 0, ULBZVal = 0, 본좌임팩트MaxVal = 25, ULBZValMax = 15;
        bool StressTimerON = true, 본좌임팩트타이머ON = false, ULBZ = false;
        ...
        {
            게임시작, 드롭다운, 블록무브, 렙업, 게임오버, delOne, delTwo, delThree, delFour, 블록회전, 본좌임펙트
        };
        ...
        internal void 인터페이스자동배열(int 맵가로, int 맵세로)
        {
            ...
            lbl_본좌모드발동.Location = new Point((MainPanel.Size.Width - lbl_본좌모드발동.Width) / 2,
                (Size.Height - lbl_본좌모드발동.Height) / 2);
        ...
        internal void 난이도설정(난이도 nid)
        {
            ...
            switch (nid)
            {
                ...
                case 난이도.본좌:
                    ...
                    try
                    {
                        BGMPATH = Application.StartupPath + "\\03 대경성 (Feel The Soul) (Seotaiji 6th Album Re-recording).mp3";
                        BGMPATH = Application.StartupPath + "\\강마에_6집_-_똥덩어리땡-bclman.mp3";
                        BGM = new Audio(BGMPATH, false);
                    }
                    catch { }
                    본좌임팩트타이머ON = true;
                    ULBZ = false;
        ...
        private void EndGame()
        {
            ...
            this.왼쪽으로키 = Program.Ifrm_set.왼쪽으로키;
            this.오른쪽으로키 = Program.Ifrm_set.오른쪽으로키;
            //if (lbl_본좌모드발동.Visible)
            //{
            //    Keys tmp1 = 회전키, tmp2 = 왼쪽으로키;

            //    회전키 = 한칸내림키; //회전키에 한칸내림키저장
            //    한칸내림키 = tmp1; //한칸내림키에 회전키저장

            //    왼쪽으로키 = 오른쪽으로키; //왼쪽으로키에 오른쪽으로키
            //    오른쪽으로키 = tmp2; //오른쪽으로키에 왼쪽으로키
            //}
            if (BGM != null)
            {
                BGM.Stop();
        ...
        internal void PlayS(사운드종류 st)
        {
            ...
            switch (st)
            {
                ...
                case 사운드종류.본좌임펙트:
                    SP[4].SoundLocation = curL + "본좌임펙트.wav";
                    break;
        ...
        private void MFTimer_Tick(object sender, EventArgs e)
        {
            if (StressTimerON)
            {
                if (StressValue == 3600)
                {
                    ...
                    if (MessageBox.Show("1시간이 지났습니다.\n건강을 위해서 좀 쉬었다하는건 어떨까요?", "피로도 시스템",
            ...
            {
                ULBZ = false;
                if (본좌임팩트Val == 본좌임팩트MaxVal)
                {
                    //if (I난이도 == 난이도.본좌)
                    //{                        
                        Random r = new Random();

                        if (r.Next(0, 5) == 0) // 5분의1
                        {
                            lbl_본좌모드발동.Visible = true;
                            Keys tmp1 = 회전키, tmp2 = 왼쪽으로키; //회전키, 왼쪽으로 키 임시저장
                            ...
                            BlockDownTimer.Interval = (BlockDownTimer.Interval / 2);
                            SpeedLabel.Text = BlockDownTimer.Interval.ToString() + "ms";
                            ...
                            //ULBZValMax  = (ushort)r.Next(10, 15);
                            //본좌임팩트MaxVal = (ushort)r.Next(ULBZValMax + 1, 25);
                        //}
                    }
            ...
            if (BGMP)
            {
                if (BGM != null)
                {
                    if (BGM.CurrentPosition == BGM.Duration)
                        BGM.CurrentPosition = 0.0d;
                }
