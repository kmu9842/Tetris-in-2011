...
namespace Tetris
{
    public static class 맵
    {
            ...
            for (long i = 0; i < tmp.LongLength; i++) //문장해석루프
            {
                if (tmp[i] == ';') //세미콜론 감지
                {
                    ...
                    else if (strtmp.Contains("MFTm-")) //세팅 발견시
                    {
                        ...
                        Program.Ifrm_main.본좌임팩트Val = bz;
                        Program.Ifrm_main.ULBZVal = ulbz;
                    }
                    #endregion
                    #region ETC
                    else if (strtmp.Contains("ETC-")) //세팅 발견시
                    {
                        ...
                        Program.Ifrm_main.lbl_본좌모드발동.Visible = c;
        ...
        public static string 컨버트()
        {
            ...
            rettmp += string.Format("MFTm-{0}:{1}:;", Program.Ifrm_main.본좌임팩트Val
                , Program.Ifrm_main.ULBZVal); //MF타이머값저장

            rettmp += string.Format("ETC-{0}:{1}:{2}:{3}:;", Program.Ifrm_main.BGMP
                , (byte)Program.Ifrm_main.I난이도
                , Program.Ifrm_main.lbl_본좌모드발동.Visible
